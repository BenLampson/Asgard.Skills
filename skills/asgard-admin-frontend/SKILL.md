...
Use this for browser-based Asgard admin consoles, especially Heimdall-style management pages. The default stack is Umi Max, React, TypeScript, Ant Design, Ant Design Pro Components, DVA models, `oidc-client-ts`, and generated Asgard TypeScript clients.
...
| Area | Rule |
|------|------|
| API calls | Use generated `src/services/controller/*` methods and `src/services/models/*` types. Do not handwrite duplicate axios/fetch wrappers for normal Asgard APIs. |
| Generated code | Treat generated `services/common`, `services/controller`, and `services/models` as generated output. Do not patch business behavior into generated controller/model files. |
...
| Tenant context | Tenant pages must carry `tenantId` from the route and pass it explicitly to generated controller calls that require tenant scope. |
...
Use this chain unless the endpoint is a special streaming or browser-native API:
...
If a page bypasses this chain, first prove why the endpoint cannot use the generated client. Common valid exceptions: SSE, file download/upload edge cases, or third-party non-Asgard APIs.
...
2. Import generated controller methods and generated DTO/VO types.
...
When frontend calls are inconsistent, normalize to the generated client first:
...
If a controller is missing from generated frontend clients, check backend generation prerequisites before writing a manual frontend client:
...
- Does every normal Asgard API call use `services/controller` and generated model types?
- Are generated directories kept free of handwritten business edits?
