...
目标是返回与用户分页元素相同的对象，用于创建或重新启用业务 Profile 时确认用户存在且最终状态 Enabled，尤其适用于没有指定主目录组的场景。

在此接口正式实现、测试、提交并部署前，它仍是待补能力。不要通过遍历分页验证单个用户，也不要在目录组为空时跳过 Heimdall 校验。
...
- Profile 创建/启用：使用单用户接口；接口尚未交付时 Fail Closed。
