...
|---|---|
| Heimdall 版本 | `5.1.0` |
| Git commit | `8f4e579b6a512a9b9b505119aa333040a8955543` |
| Git tag | `v5.1.0` |
| Docker | `registry.cn-hangzhou.aliyuncs.com/benlampson/asgard.heimdall:5.1.0` |
| Image digest | `sha256:b6c233396c39d635963138e7932e5241f8b124975c573a5a4738dfccff7a4f6a` |

部署时优先固定 `registry.cn-hangzhou.aliyuncs.com/benlampson/asgard.heimdall:5.1.0@sha256:b6c233396c39d635963138e7932e5241f8b124975c573a5a4738dfccff7a4f6a`，不要把可变的 `latest` 当作验收版本。镜像已发布不等于某个开发或生产环境已完成部署，环境状态需要单独记录。

`5.1.0` 正式镜像已包含以下原待发布能力：
...
发布方验证记录：后端 Release 测试 `470/470`、前端 TypeScript 检查与生产构建通过，并已从 Registry 核对 `5.1.0` 与 `latest` 指向上述相同 digest。这些自动化验证不替代接入方真实端到端验收。

当前发布记录尚未包含从 `5.1.0` 运行实例导出的 OpenAPI 静态快照，也尚未完成目标接入环境的真实 `Client -> Token -> Directory API -> identity.subject.invalidated Webhook` 联调。接入方应固定上述 digest，导出并留存 OpenAPI，再按 `end-to-end-acceptance.md` 执行联调；记录完成前只能声明“发布制品已交付”，不能声明“目标环境验收完成”。
...
| 5.1.0 OpenAPI 静态快照 | 待接入验收 | 从固定 digest 的运行实例导出并记录版本或哈希 |
